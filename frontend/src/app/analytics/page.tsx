...
export default function AnalyticsPage() {
  ...
  return (
    <div className="p-8 space-y-8 animate-in fade-in duration-500">
      <div>
        <h1 className="text-2xl font-bold text-white flex items-center gap-2"><Activity className="text-blue-500" /> Sicherheitsanalysen</h1>
        <p className="text-slate-400 mt-1">Echtzeit-Metriken zu Schwachstellen über alle indexierten Container hinweg.</p>
      </div>

      <div className="grid grid-cols-4 gap-6">
        <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 shadow-lg">
          <div className="text-slate-400 text-sm mb-2 font-medium">Gesamt-CVEs</div>
          <div className="text-4xl font-bold text-white">{stats.total}</div>
        </div>
        <div className="bg-slate-800 p-6 rounded-xl border border-red-500/30 shadow-lg shadow-red-500/5">
          <div className="text-red-400 text-sm flex items-center gap-1 mb-2 font-medium"><ShieldAlert className="w-4 h-4" /> Kritisch</div>
        ...
        <div className="bg-slate-800 p-6 rounded-xl border border-amber-500/30 shadow-lg shadow-amber-500/5">
          <div className="text-amber-400 text-sm flex items-center gap-1 mb-2 font-medium"><Shield className="w-4 h-4" /> Hoch</div>
        ...
        <div className="bg-slate-800 p-6 rounded-xl border border-blue-500/30 shadow-lg shadow-blue-500/5">
          <div className="text-blue-400 text-sm flex items-center gap-1 mb-2 font-medium"><ShieldCheck className="w-4 h-4" /> Mittel</div>
          <div className="text-4xl font-bold text-blue-500">{stats.medium}</div>
        </div>
      </div>

      <div className="bg-slate-800 p-8 rounded-xl border border-slate-700 shadow-xl">
        <h2 className="text-lg font-semibold text-white mb-6">Risikoverteilungs-Engine</h2>
        ...
        <div className="flex gap-6 mt-4 text-sm font-medium text-slate-400">
          <span className="flex items-center gap-2"><div className="w-3 h-3 rounded bg-red-500 shadow-sm"></div> Kritisch ({(stats.critical / (stats.total || 1) * 100).toFixed(1)}%)</span>
          <span className="flex items-center gap-2"><div className="w-3 h-3 rounded bg-amber-500 shadow-sm"></div> Hoch</span>
          <span className="flex items-center gap-2"><div className="w-3 h-3 rounded bg-blue-500 shadow-sm"></div> Mittel</span>
          <span className="flex items-center gap-2"><div className="w-3 h-3 rounded bg-emerald-500 shadow-sm"></div> Niedrig</span>
        </div>
      </div>
    </div>
  );
